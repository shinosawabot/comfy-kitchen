 ...
 * https://github.com/flashinfer-ai/flashinfer/blob/v0.1.5/include/flashinfer/cp_async.cuh
 ...
 * copy data from global memory to shared memory \tparam prefetch_mode Whether
 * to fetch additional data from global memory to L2 \tparam T Data type \param
 * smem_ptr Pointer to shared memory \param gmem_ptr Pointer to global memory
 */
...
/*!
 * \brief Wrapper of PTX cp.async.cg.shared.global instruction, asynchronously
 * copy data from global memory to shared memory with predicate. \tparam
 * prefetch_mode Whether to fetch additional data from global memory to L2
 * \tparam fill_mode Whether to fill zero to shared memory when predicate is
 * false \tparam T Data type \param smem_ptr Pointer to shared memory \param
 * gmem_ptr Pointer to global memory \param predicate Predicate value \note fill
 * zero is slower than not fill zero
 */
