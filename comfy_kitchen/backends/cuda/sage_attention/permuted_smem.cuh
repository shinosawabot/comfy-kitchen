 ...
 * https://github.com/flashinfer-ai/flashinfer/blob/v0.1.5/include/flashinfer/permuted_smem.cuh
...
enum class SwizzleMode {
  ...
  k128B, // 128B already spans all banks in shared memory. a line of shared
         // memory can have multiple 128B.
};
...
/*!
 * \brief A stateless shared memory wrapper that uses templates to avoid runtime
 * conditionals. It makes sure that access to consecutive rows idx in the same
 * column idx will make full use of the shared memory bank through permutation
 * in the granularity of 128bit.
 *
 * This struct treats all offsets to be the number of `b128_t` elements. It is
 * designed to be stateless, meaning it does not maintain any information about
 * the current pointer position. The offset returnd by the struct can be used to
 * access the shared memory through the provided interface.
 *
 * The struct guarantees that the read to permuted offset (i, j) will be the
 * value stored in permuted offset (i, j). We assume that shared memory
 * operation operates on at least two consecutive 128-bit values in a row within
 * a warp. Under this assumption, we do not permute for k32B mode.
 */
template <SwizzleMode swizzle_mode, uint32_t stride> struct smem_t {
  // The base pointer.
  b128_t *base;
  ...
  __device__ __forceinline__ smem_t(T *base) : base((b128_t *)base) {
    if constexpr (swizzle_mode == SwizzleMode::k128B) {
      static_assert(stride % 8 == 0,
                    "Stride must be multiple of 8 for 128B swizzle mode");
   ...
   */
  template <typename T> __device__ __forceinline__ void set_base(T *new_base) {
    base = (b128_t *)new_base;
  }

  /*!
   * \brief Compute the element offset given coordinates in a permuted shared
   * memory. \param i The row index. \param j The column index.
   */
  static __device__ __forceinline__ uint32_t
  get_permuted_offset(const uint32_t &i, const uint32_t &j) {
    if constexpr (swizzle_mode == SwizzleMode::k128B) {
  ...
  /*!
   * \tparam step_size The step size to advance the offset in the permuted
   * shared memory. \param offset The current offset.
   */
  template <uint32_t step_size>
  static __device__ __forceinline__ uint32_t
  advance_offset_by_column(const uint32_t &offset) {
    if constexpr (swizzle_mode == SwizzleMode::k128B) {
      static_assert(step_size % 8 == 0, "Unsupported step size");
  ...
  template <uint32_t step_size>
  static __device__ __forceinline__ uint32_t
  advance_offset_by_column(const uint32_t &offset, const uint32_t &step_idx) {
    if constexpr (swizzle_mode == SwizzleMode::k128B) {
  ...
  template <uint32_t step_size>
  static __device__ __forceinline__ uint32_t
  advance_offset_by_row(const uint32_t &offset) {
    if constexpr (swizzle_mode == SwizzleMode::k128B) {
  ...
  __device__ __forceinline__ void ldmatrix_m8n8x2(const uint32_t &offset,
                                                  uint32_t *R) const {
    b128_t *smem_ptr = base + offset;
    mma::ldmatrix_m8n8x2(R, smem_ptr);
  }

  __device__ __forceinline__ void ldmatrix_m8n8x4(const uint32_t &offset,
                                                  uint32_t *R) const {
    b128_t *smem_ptr = base + offset;
    mma::ldmatrix_m8n8x4(R, smem_ptr);
  }

  __device__ __forceinline__ void ldmatrix_m8n8x4_trans(const uint32_t &offset,
                                                        uint32_t *R) const {
    b128_t *smem_ptr = base + offset;
  ...
  template <typename T>
  __device__ __forceinline__ void store_128b(const uint32_t &offset,
                                             T *gptr) const {
