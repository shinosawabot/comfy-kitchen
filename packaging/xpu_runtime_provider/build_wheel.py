#!/usr/bin/env python3
"""Build the co-installable Comfy Kitchen XPU runtime provider wheel."""
...
CANONICAL_PACKAGE = "comfy_kitchen"
...
def _source_wheel_contract(
    source_wheel: Path,
) -> tuple[str, str, tuple[str, ...], dict[str, bytes]]:
    ...
    if f"{CANONICAL_PACKAGE}/__init__.py" not in files:
        raise RuntimeError(
            f"source wheel does not contain {CANONICAL_PACKAGE}/__init__.py"
        )
    return version, requires_python, tags, files
...
def build_provider_wheel(
    *,
    ...
    """Re-home a canonical wheel below a private provider-owned package."""
    ...
    contents[f"{PROVIDER_PACKAGE}/__init__.py"] = (
        '"""Co-installable Comfy Kitchen XPU runtime provider."""\n'
        "\n"
    ...
    contents[f"{dist_info}/METADATA"] = (
        ...
        "Summary: Co-installable Comfy Kitchen Intel XPU runtime provider\n"
        f"Requires-Python: {requires_python}\n"
        "\n"
    ).encode("utf-8")
    contents[f"{dist_info}/WHEEL"] = (
        "Wheel-Version: 1.0\n"
        "Generator: comfy-kitchen-xpu-runtime-provider\n"
    ...
    filename = (
        f"{_wheel_component(PROVIDER_DISTRIBUTION)}-"
        f"{_wheel_component(source_version)}-{tag}.whl"
    )
